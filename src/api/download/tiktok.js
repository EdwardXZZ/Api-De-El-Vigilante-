...
module.exports = function(app) {
    ...
    async function scrapeTiktokV2(url) {
        try {
    ...
    async function tiktokDownload(url) {
        try {
            const result = await scrapeTiktokV2(url);

            if (result.error) {
                throw new Error(result.message || 'Failed to scrape TikTok data');
            }
            ...
            let output = {
                ...
                likes: metadata.stats.likeCount,
                comments: metadata.stats.commentCount,
                shares: metadata.stats.shareCount,
                views: metadata.stats.playCount
            };

            if (download.photo && download.photo.length > 0) {
                output.images = download.photo;
                output.audio = download.audio;
            } else if (download.video && download.video.length > 0) {
                if (!download.video) throw new Error('No se pudo obtener el video');
                output.video = download.video[0];
                output.audio = download.audio;
            } else {
        ...
        try {
            ...
            if (req.query.download === 'true') {
                if (result.video) {
                    ...
                    return res.redirect(result.images[0]);
                }
